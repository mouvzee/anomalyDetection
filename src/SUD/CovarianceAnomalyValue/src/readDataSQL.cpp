...
bool readDataSQL(std::map<std::int32_t, std::vector<Data>> &dataVector, std::map<std::int32_t, std::vector<Average>> &averages, std::vector<std::vector<std::vector<Covariance>>> &covariances, PGconn *conn){
    ...
    for (int i = 0; i <PQntuples(resSensorsID); i++) {
        ...
        for (int j = 0; j <PQntuples(resSensorData); j++) {
            ...
            dataVector[std::stoi(sensorID)].push_back(data);
        ...
        // Scorrimento delle medie del sensore per finestra temporale e creazione e popolazione della struttura dati per esse
    ...
    for(size_t i = 0; i<dataVector.size(); i++){
        ...
        std::string sensor1 = std::to_string(sensors[i]);

        for(size_t j = 0; j<dataVector.size(); j++){
            
            covariances[i].emplace_back();
            std::string sensor2 = std::to_string(sensors[j]);
